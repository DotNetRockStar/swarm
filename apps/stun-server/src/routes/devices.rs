...
fn validate_registration(device: &DeviceRegistration) -> Result<(), AppError> {
    ...
    if fp.len() != 64 || !fp.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
        return Err(AppError::bad_request("invalid_fingerprint", "cert_fingerprint must be 64 lowercase hex chars"));
    }
    if device.name.trim().is_empty() || device.name.len() > 64 {
        return Err(AppError::bad_request("invalid_name", "device name must be 1-64 characters"));
    }
    if device.machine_id.trim().is_empty() || device.machine_id.len() > 128 {
        return Err(AppError::bad_request("invalid_machine_id", "machine_id must be 1-128 characters"));
    }
    if device.metadata.len() > 64 {
        return Err(AppError::bad_request("too_much_metadata", "at most 64 metadata keys"));
    }
    for (key, value) in &device.metadata {
        if key.is_empty() || key.len() > 64 || value.len() > 1024 {
            return Err(AppError::bad_request("invalid_metadata", "metadata keys 1-64 chars, values <= 1024 chars"));
        }
    }
    Ok(())
}
...
async fn redeem_code(state: &SharedState, code: &str, device_id: &str) -> ApiResult<SwarmSummary> {
    ...
    let Some((code_id, swarm_id, swarm_name)) = row else {
        return Err(AppError::unauthorized("join code is invalid, expired, or already used"));
    };
    let claimed = sqlx::query("UPDATE join_codes SET redeemed_by_device = ? WHERE id = ? AND redeemed_by_device IS NULL")
        .bind(device_id)
        .bind(&code_id)
        .execute(&state.db)
        .await?;
    ...
    Ok(SwarmSummary { id: swarm_id, name: swarm_name })
}

async fn add_membership(state: &SharedState, swarm_id: &str, device_id: &str) -> ApiResult<()> {
    sqlx::query("INSERT OR IGNORE INTO swarm_devices (swarm_id, device_id, joined_at) VALUES (?, ?, ?)")
        .bind(swarm_id)
        .bind(device_id)
        .bind(now())
        .execute(&state.db)
...
async fn replace_metadata(state: &SharedState, device_id: &str, metadata: &BTreeMap<String, String>) -> ApiResult<()> {
    sqlx::query("DELETE FROM device_metadata WHERE device_id = ?").bind(device_id).execute(&state.db).await?;
...
pub async fn register(
    ...
    let swarm = redeem_code(&state, &req.code, &device_id).await?;
    add_membership(&state, &swarm.id, &device_id).await?;
    replace_metadata(&state, &device_id, &req.device.metadata).await?;
    state.blocker.record_success(addr.ip());
    tracing::info!(device_id, swarm = %swarm.name, "device registered");
    Ok((
        axum::http::StatusCode::CREATED,
        Json(RegisterDeviceResponse { access_token, device_id, swarm }),
    ))
}
...
pub async fn join_swarm(
    ...
    let swarm = match redeem_code(&state, &req.code, &device.device_id).await {
        Ok(swarm) => swarm,
        Err(err) => {
            state.blocker.record_failure(addr.ip());
            return Err(err);
    ...
    add_membership(&state, &swarm.id, &device.device_id).await?;
    Ok(Json(swarm))
}

#[utoipa::path(get, path = "/api/v1/me/devices", responses((status = 200, body = MyDevicesResponse)), tag = "devices")]
pub async fn my_devices(State(state): State<SharedState>, jar: CookieJar) -> ApiResult<Json<MyDevicesResponse>> {
...
pub async fn revoke(
    ...
    sqlx::query("DELETE FROM swarm_devices WHERE device_id = ?").bind(&device_id).execute(&state.db).await?;
    state.hub.send_to(
        &device_id,
        swarm_core::signal::SignalMessage::Error { code: "revoked".into(), message: "device was revoked".into() },
    );
...
pub async fn patch_metadata(
    ...
    if device.device_id != device_id {
        return Err(AppError::forbidden("a device may only update its own metadata"));
    }
