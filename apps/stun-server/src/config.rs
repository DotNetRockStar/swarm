...
impl SmtpConfig {
    ...
    fn from_env() -> Option<Self> {
        ...
        Some(Self {
            ...
            implicit_tls: std::env::var("SWARM_SMTP_STARTTLS").map(|v| v != "true").unwrap_or(true),
            from_email: std::env::var("SWARM_EMAIL_FROM").expect(
                "SWARM_EMAIL_FROM is required when SWARM_SMTP_HOST is set",
            ),
            from_name: std::env::var("SWARM_EMAIL_FROM_DISPLAY_NAME").unwrap_or_else(|_| "SWARM".into()),
        })
    }
}
