...
fn compressed_manifest_request() -> PeerRequest {
    PeerRequest {
        path: "/catalog/manifest.gz".into(),
        range: None,
        if_none_match: None,
        playback: None,
        error_report: None,
...
async fn liking_persists_and_counts_toward_the_manifest() {
    ...
    let response = fx
        .service
        .resolve(&toggle_request(Some(sample_like(&fx.entry_key, true))))
        .await;
    ...
    let swarm_media::serve::Body::Bytes(body) = manifest_response.body else {
    ...
    let manifest_entry = manifest
        .entries
        .iter()
        .find(|e| e.entry_key == fx.entry_key)
        .unwrap();
    assert_eq!(manifest_entry.like_count, 1);
}

#[tokio::test]
async fn compressed_manifest_round_trips_as_gzip_json() {
    let fx = fixture().await;
    let response = fx.service.resolve(&compressed_manifest_request()).await;
    assert_eq!(response.header.status, 200);
    assert_eq!(
        response.header.content_type.as_deref(),
        Some("application/gzip")
    );
    let swarm_media::serve::Body::Bytes(body) = response.body else {
        panic!("expected an in-memory body")
    };
    ...
    GzDecoder::new(body.as_slice())
        ...
        .unwrap();
    let manifest: swarm_core::peer::CatalogManifest = serde_json::from_slice(&json).unwrap();
    assert_eq!(manifest.entries.len(), 1);
    assert_eq!(manifest.entries[0].entry_key, fx.entry_key);
}
...
async fn unliking_removes_the_like_and_is_idempotent() {
    ...
    let first = fx
        .service
        .resolve(&toggle_request(Some(sample_like(&fx.entry_key, false))))
        .await;
    ...
    assert_eq!(
        fx.library.like_counts().await.unwrap().get(&fx.entry_key),
        None
    );
}
...
async fn two_devices_liking_the_same_entry_both_count() {
    ...
    fx.service
        .resolve(&toggle_request(Some(second_device)))
        .await;
...
async fn request_with_an_empty_device_id_or_entry_key_is_rejected() {
    ...
    assert_eq!(
        fx.service
            .resolve(&toggle_request(Some(missing_device)))
            .await
            .header
            .status,
        400
    ...
    assert_eq!(
        fx.service
            .resolve(&toggle_request(Some(missing_entry_key)))
            .await
            .header
            .status,
        400
